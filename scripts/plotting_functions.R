...
#' Calculate extended limits so that fullrange = TRUE
extend_limits <- function(x, include = c(), mult = 0.05) {
  range(c(include, x), na.rm = TRUE) + c(-1, 1) * mult * 
    diff(range(c(include, x), na.rm = TRUE))
}
