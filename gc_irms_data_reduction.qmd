...
standards <- 
  ...
  # I don't trust these standards, nC16 is the first peak and systematically off and nC24 is usually an outlier
  #filter(!compound %in% c("nC16", "nC24")) |>
...
peak_table_for_calib <- 
  ...
  left_join(
    standards |> 
      select("compound", "n_d2H", "known_d2H.permil", "known_d2H_sd.permil") |>
...
```{r}
#| label: calc-s6-stats
# unbiased sds for standards (within the calib area)
calib_compounds_stats <- 
  peak_table_calibrated |>
  bind_rows(peak_table_calibrated |> mutate(session = "all")) |>
  filter(use_to_calibrate) |>
  summarize(
    .by = c("session", "compound", "known_d2H.permil"),
    n_analyses = n(),
...
# calculate averages and error ranges
peak_table_summary <- 
  peak_table_calibrated |>
  filter(type != "standard", str_detect(compound, "BP")) |>
  summarize(
...
### Accuracy standard: nC36 (skip?)

```{r}
#| label: fig-calibration-accuracy
#| fig-width: 6
#| fig-height: 6
...
peak_table_calibrated |>
  bind_rows(peak_table_calibrated |> mutate(session = "all")) |>
  filter(compound == "nC36", in_area_range) |>
  summarize(
    .by = c("session", "known_d2H.permil"),
    n = n(),
    calib_mean.permil = mean(calibrated_d2H.permil),
    unbiased_sd.permil = calculate_unbiased_sd(sd(calibrated_d2H.permil), n),
    diff.permil = mean(residual.permil),
    RMSE = sqrt(mean(residual.permil^2))
  ) |>
...
# visualize
peak_table_calibrated |>
  filter(compound == "nC36") |>
  ggplot() +
  aes(area.Vs, residual.permil, 
      ymin = residual.permil - calibrated_d2H_err.permil,
      ymax = residual.permil + calibrated_d2H_err.permil,
      color = session, shape = session
    ) +
  ...
  geom_hline(
    data = ~.x |> filter(in_area_range) |> 
      summarize(.by = "session", y = mean(residual.permil)),
    map = aes(yintercept = y, x = NULL, color = session), 
    linetype = 1, linewidth = 0.5
  ) +
  # data
  geom_errorbar(data = ~.x |> filter(!in_area_range), width = 0, alpha = 0.5) +
  geom_point(data = ~.x |> filter(!in_area_range), size = 4, alpha = 0.5) +
  geom_errorbar(data = ~.x |> filter(in_area_range), width = 0) +
  geom_point(data = ~.x |> filter(in_area_range), size = 4) +
  # styling
  labs(
    x = "area 2 [Vs]",
    y = expression("error ("*delta[calib] - delta[known]*") for accuracy standard(nC"[36]*")")
  ) +
  scale_x_log10(breaks = scales::breaks_log(8)) + 
  scale_y_continuous(
    labels = function(x) paste0(x, "\U2030"),
    breaks = seq(-30, 30, by = 2.5)
  ) +
  scale_color_brewer(palette = "Dark2") +
  theme_figure(grid = FALSE) +
  theme(legend.position = "bottom", legend.direction = "vertical") +
  coord_cartesian(
    xlim = peak_table_calibrated |>
      filter(compound == "nC36") |>
      filter(in_area_range) |> 
      pull(area.Vs) |> extend_limits(mult = c(0.03, 0.3)),
    ylim = peak_table_calibrated |>
      filter(compound == "nC36") |>
      filter(in_area_range) |> 
      pull(residual.permil) |> extend_limits(mult = 0.2)
  )
```
...
peak_table_calibrated |>
  ...
  summarize(
    .by = "session",
    # FIXME: find a better way to expand the range
    area.Vs = list(seq(min(area.Vs)^0.9, max(area.Vs)^1.1, length.out = 100)),
    d2H.permil = list(seq(min(calibrated_d2H.permil), max(calibrated_d2H.permil), length.out = 100)),
  ) |>
  unnest("area.Vs") |> unnest("d2H.permil") |>
  mutate(
    .by = "session",
    calibrated_d2H_sem.permil =
      calculate_calibrated_d2H_se.permil(session[1], d2H.permil, area.Vs)
  ) |>
  ggplot() +
  aes(area.Vs, d2H.permil, z = calibrated_d2H_sem.permil) +
  geom_contour_filled() +
  scale_x_log10(breaks = scales::breaks_log(8)) +
  coord_cartesian(expand = FALSE) + 
